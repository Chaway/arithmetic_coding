...
void main(int argc , char * args[])
{
 ...
 if((fp = fopen(args[1] , "rb"))== NULL)    //open binary file
  ...
  //initialize
  char symbols[symbol_num + 1];
  int Count[symbol_num];
  for(int i = 1 ;i <= symbol_num ; ++i)
  {
     fscanf(fs,"%d %d", &symbols[i] , &Count[i - 1]);
     printf("number %d symbol :%c  (ASCII value = %d) , Count[%d] = %d \n", i , symbols[i] , symbols[i], i - 1 , Count[i - 1]);
  }
           ...
           if(bit.UMSB == bit.LMSB)
           {
            ...
            else
            {
               ...
               else
               {
                 ...
                 printf("@@@@@@current_symbol is  %c (ASCII = %d)\n",symbol,symbol);
                 fputc(symbol , fpo);
